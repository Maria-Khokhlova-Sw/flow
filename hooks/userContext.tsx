...
export const UserProvider = ({ children }: UserProviderProps) => {
    ...
    const initialConversations: Conversations = useMemo(
        () => ({
            1: [
            ...
            3: [
                ...
                { id: "3-3", author: "them", text: "Утро доброе :)", timestamp: BASE_TIMESTAMP - 1000 * 60 * 60 * 11.8, status: "sent" },
            ],
        }),
        []
    )
    ...
    const sendMessage = useCallback((userId: number, text: string) => {
        if (!text.trim()) return
    ...
    const markMessagesAsRead = useCallback((userId: number) => {
        setConversations((prev) => {
    ...
    const value: UserContextType = useMemo(
        ...
        [users, conversations, selectedUserId, selectUser, sendMessage, markMessagesAsRead]
    )
